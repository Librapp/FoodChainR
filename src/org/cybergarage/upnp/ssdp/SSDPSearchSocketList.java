...
public class SSDPSearchSocketList extends Vector {
	 ...
	 * @since 1.8
	 */
	public SSDPSearchSocketList(InetAddress[] binds, int port,
			String multicastIPv4, String multicastIPv6) {
		this.binds = binds;
		this.port = port;
		...
		for (int i = 0; i < bindAddresses.length; i++) {
			if (bindAddresses[i] != null) {
				...
				if (!HostInterface.isIPv6Address(bindAddresses[i])) {
					// ssdpSearchSocket = new
					// SSDPSearchSocket(bindAddresses[i],port ,multicastIPv6 );
					// else
					ssdpSearchSocket = new SSDPSearchSocket(bindAddresses[i],
							port, multicastIPv4);
